...
class Student(BaseModel):
    ...
    def validate_emergency_contact(cls, model):
        if model.age > 20 and model.married == False:
            return ValueError(f"{model.name} is a unmarried major!! with marital status {model.married}")
...
student_info = {'name': 'Ani', 'age': 25, "roll": 26, 'email': 'lhamsani@gmail.com', 'married': True, 'leaves': [1,3,4,5,6,7], 'subjects': {'math': 9, 'eng': 10, 'science': 11}, 'weight': 99.1, 'height': 185.0}
...
def insert_student_data(student: Student):
    ...
    print()
    print('BMI of {student.name} whose height {student.height} and weight {student.weight} is :', student.bmi())
